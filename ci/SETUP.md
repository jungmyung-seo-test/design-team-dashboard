...
| `BOARD_PASSWORD` | 팀에 공유한 보드 비밀번호 (값은 여기 적지 않는다 · `ci/genpw.py` 로 생성) |
| `BOARD_CONFIG` | `ci/BOARD_CONFIG.json` **파일 내용 전체**를 그대로 붙여넣기 |
| `REFRESH_KEY` | **선택.** 갱신 버튼용 공유 키 — 아래 설명 참고 |

`BOARD_CONFIG` 는 명부·소속·ONE 그룹·지라 조회 조건이 모두 들어 있는 JSON 이다.
인원이 바뀌면 **이 시크릿만** 고치면 된다. 코드는 건드리지 않는다.

`REFRESH_KEY` 는 넣지 않아도 된다. 넣으면 그 값이 암호화된 보드 안에 들어가고,
버튼이 갱신을 요청할 때 함께 보낸다. Worker 쪽에도 같은 값을 넣으면(5-4) 갱신을
걸 수 있는 사람이 **"Worker 주소를 아는 사람"에서 "보드 비밀번호를 아는 사람"으로**
좁혀진다. 비워 두면 Worker 가 키 검사를 건너뛴다.
...
40초쯤 뒤 보드가 새로 배포되면 파이프라인이 정상이다. (실측 34~43초)
...
처음 배포할 때 workers.dev 서브도메인 등록과 Preview URL 사용 여부를 묻는다.
서브도메인은 등록하고, **Preview URL 은 끈다**(`wrangler.toml` 에 이미
`preview_urls = false` 로 적어 뒀다) — 켜 두면 배포할 때마다 갱신을 걸 수 있는
공개 주소가 하나씩 늘어난다.
...
Worker 엔드포인트에는 인증이 없다. 주소를 아는 사람은 누구나 갱신을 실행시킬 수
있다. 데이터는 볼 수 없고 피해는 지라 조회 부하뿐이며, 아래 쿨다운이 상한을
잡는다. 그래도 조이고 싶으면 양쪽에 같은 값을 넣는다.

```bash
cd ci/worker && npx wrangler secret put REFRESH_KEY
```

GitHub 쪽에도 **같은 값**을 `REFRESH_KEY` 시크릿으로 넣고 워크플로를 한 번 돌린다.
한쪽만 넣으면 안 된다 — Worker 에만 넣으면 모든 요청이 `401` 로 막히고,
GitHub 에만 넣으면 아무 효과가 없다.

### 5-5. 연타 방지 — 쿨다운

직전 갱신이 끝난 지 **30초** 안에 다시 누르면 워크플로를 돌리지 않고
`방금 갱신됨` 으로 답한다. 이미 실행 중이면 그 실행을 함께 기다린다.
26명이 동시에 눌러도 한 번만 돈다.

값을 바꾸려면 `wrangler.toml` 에 `COOLDOWN_SEC` 를 넣는다. 길게 잡지 않는 게 좋다 —
"지라에서 티켓 고치고 바로 눌러 본다"가 정상적인 사용이라, 그걸 막으면 사용자는
갱신이 고장난 것으로 받아들인다.
...
| 눌러도 `401` | `REFRESH_KEY` 를 Worker 에만 넣고 GitHub 시크릿에는 안 넣었다 (5-4) |
| 고친 코드가 반영 안 됨 | 푸시 직후 바로 갱신하면 직전 커밋으로 빌드될 수 있다. 30초쯤 두었다가 다시 누른다 |
